#!/usr/bin/env python3
"""Generates C# operation usage snippets from an sdkgen.lock TypeAPI specification file."""
...
def map_schema_to_type(schema: dict) -> str:
    """Map a TypeAPI schema definition to its corresponding clean C# type name."""
    ...
    if schema_type == "reference":
        return to_pascal_case(schema.get("target", "Object"))
...
def generate_usage(lock_file_path: Path) -> str:
    """Read sdkgen.lock TypeAPI spec and generate clean C# operation snippets."""
    ...
    for op_id, op in operations.items():
        ...
        for arg_data in arguments_list:
            ...
            if param_in == "path":
                ...
                if schema.get("type") == "string":
                    ...
                    call_args.append(f"default({type_name})")

        return_spec = op.get("return", {})
        return_schema = return_spec.get("schema", {}) if isinstance(return_spec, dict) else {}
        return_type = map_schema_to_type(return_schema) if return_schema else None

        description = op.get("description", "").strip() if isinstance(op.get("description"), str) else ""
        if description:
        ...
        if return_type and return_type != "void":
            lines.append(
                f"{return_type} response = {tag_accessor}.{method_name}({call_str});"
            )
        else:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate C# usage snippets from sdkgen.lock.")
